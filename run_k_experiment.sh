...
echo "==============================================="
#java -cp $ARX_CLASSPATH:build/classes syntheticmedical.KAnonymization 2
#java -cp $ARX_CLASSPATH:build/classes syntheticmedical.KAnonymization 4
#java -cp $ARX_CLASSPATH:build/classes syntheticmedical.KAnonymization 8
#java -cp $ARX_CLASSPATH:build/classes syntheticmedical.KAnonymization 16
for ((i=2;i<=200;i++));
do
  java -cp $ARX_CLASSPATH:build/classes syntheticmedical.KAnonymization $i
done
echo "==============================================="
for ((i=2;i<=200;i++));
do
  java -cp $ARX_CLASSPATH:build/classes syntheticmedical.KAnonymizationWithSuppression $i
done
