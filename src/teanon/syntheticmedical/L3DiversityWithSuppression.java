 ...
 */
public class L3DiversityWithSuppression extends Example {
    ...
    public static void main(final String[] args) {
        if(args.length < 1) {
            System.out.println("It requires l parameter!");
            return;
        }
        ...
        try {
            ...
            data.getDefinition().setAttributeType("complaint", AttributeType.SENSITIVE_ATTRIBUTE);
            data.getDefinition().setAttributeType("classification", AttributeType.INSENSITIVE_ATTRIBUTE);

            // Create an instance of the anonymizer
            final ARXAnonymizer anonymizer = new ARXAnonymizer();
            final ARXConfiguration config = ARXConfiguration.create();
            config.addCriterion(new KAnonymity(l));
            config.addCriterion(new RecursiveCLDiversity("complaint", 3, l));
            ...
            System.out.println(l + "\t" + result.getGlobalOptimum().getMaximumInformationLoss() + 
                                    "\t" + result.getGlobalOptimum().getTotalGeneralizationLevel());
...
//            result.getOutput(false).save("dataset/anonymized_flash_k.csv", '|');
